image:post-logo-apache-camel-d.png[Apache Camel K,role="center"]
...
First, we need to create a simple Java Camel K example. You can do this using the `Create a new Apache Camel K Integration file` command. (link:didact://?commandId=camelk.integrations.createNewIntegrationFile&text=HelloWorld$$Java[Execute^])
