	...
	if (language) {
		...
		if (workspaceFolder) {
			...
			if (filename) {
				...
				let newFileFullPath: string = computeFullpath(language, workspaceFolder, filename);
...
function computeFullpath(language: string, workspaceFolder: vscode.WorkspaceFolder, filename: string) {
