...
suite("VersionUtils check", () => {
	...
	suite("ensure version url methods are functioning as expected", () => {
		...
		test("validate url for existing 1.4.1 windows version", async () => {
			await validateVersion('1.4.1', Platform.WINDOWS, 'https://github.com/apache/camel-k/releases/download/v1.4.1/camel-k-client-1.4.1-windows-64bit.tar.gz');
		});

		test("validate url for existing 1.4.1 MacOS version", async () => {
			await validateVersion('1.4.1', Platform.MACOS, 'https://github.com/apache/camel-k/releases/download/v1.4.1/camel-k-client-1.4.1-mac-64bit.tar.gz');
		});
