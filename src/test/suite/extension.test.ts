...
describe("ensure camelk extension exists and is accessible", function() {
	...
	describe('namespace kamel support', function() {
	...
	describe('namespace setting tests', function() {

		var stashedNS : string | undefined = undefined;

		this.beforeEach( function () {
			stashedNS = config.getNamespaceconfig();
		});

		this.afterEach( async function() {
			// reset to old value
			await config.addNamespaceToConfig(stashedNS);			
		});

		test('test setting namespace to undefined', async function() {
			// reset to undefined
			await config.addNamespaceToConfig(undefined).then( () => {
